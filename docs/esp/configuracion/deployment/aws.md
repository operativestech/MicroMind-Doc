...
You can deploy aimicromind on AWS ECS using our CloudFormation template. The template will create:
...
4. Upload the `cloudformation.yaml` file from the [AiMicromind repository](https://github.com/operativestech/AiMicroMind_Platform_2025/blob/main/docker/cloudformation.yaml)
5. Click **Next**
...
After deployment, the URL of your aimicromind application will be available in the CloudFormation stack outputs.

## Deploy on ECS using Terraform

The Terraform files (`variables.tf`, `main.tf`) are available in this GitHub repository: [terraform-aimicromind-setup](https://github.com/huiseo/terraform-aimicromind-setup/tree/main).
...
```bash
git clonehttps://github.com/operativestech/AiMicroMind_Platform_2025.git
```
...
5. _(Optional)_ You can also specify `AIMICROMIND_USERNAME` and `AIMICROMIND_PASSWORD` for app level authorization. See more [broken-reference](broken-reference/ "mention")
...
4. <pre class="language-bash"><code class="lang-bash"><strong>sudo nano /etc/nginx/conf.d/aimicromind.conf
   </strong></code></pre>
...
You have successfully setup aimicromind apps on EC2 instance with SSL certificate on your domain[🥳](https://emojipedia.org/partying-face/)
